...
##取2005-2016年HS300股票数据 每年6月末进行调整
...
def size_bm_group(breakyear):
    ...
    string1='./hs300_weights_'
    string2='.xls'
    hs_file="%s%s%s"%(string1,breakyear,string2)
    #hs_file='hs300_weights_'+breakyear+'.xls'
    hs300s=pd.read_excel(hs_file)
    stock_file="%s%s%s"%('./stock_',breakyear,'.xls')
    #stock_file='/stock_'+breakyear+'.xls'
